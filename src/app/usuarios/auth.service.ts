...
export class AuthService {
  ...
  login(usuario:Usuario):Observable<any>{
    ...
    return this.http.post<any>(urlEndPoint, params.toString(), {headers : httpHeaders}).pipe(
      catchError(e => {
        if(e.status == 400 && e.error.error_description == 'Bad credentials'
              && e.error.error == 'invalid_grant'){
                swal('Error en login', 'Credenciales no validas', 'error');
        }else{
          swal('Error en login', `Error no esperado: Error: ${e.error.error} Description: ${e.error.error_description}`)
        }
        return throwError(e);
      })
    );
